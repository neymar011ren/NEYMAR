...
def _augmented_search_path() -> str:
    """run_agent_install 可能把可执行文件装到用户级目录（如 npm_config_prefix 兜底、
    pip --user），但那只体现在子进程的 env 里，不会写回当前进程的 os.environ。
    这里把这些常见的用户级 bin 目录一并纳入 shutil.which 的搜索范围，
    避免安装成功后立刻检测却因为当前进程 PATH 没更新而误报「未安装」。
    """
    extra_dirs = [
    ...
    extras = os.pathsep.join(str(d) for d in extra_dirs if d.exists())
    return f"{extras}{os.pathsep}{current}" if extras else current


def check_agent_installed(agent_id: str) -> dict[str, Any]:
    profile = resolve_profile(agent_id)
    if not profile:
        # 和 probe_agent_config / write_agent_channel_config 保持一致的错误契约：
        # 无法识别的 agent_id 应该明确 ok=False，而不是伪装成"合法但未安装"。
    ...
    if recipe:
        search_path = _augmented_search_path()
        for b in recipe.check_bins:
            resolved = shutil.which(b, path=search_path)
            if resolved:
    ...
    return {
        ...
        "docs_url": recipe.docs_url if recipe else profile.docs_hint,
