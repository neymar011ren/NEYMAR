...
def check_protocol_fidelity(native_protocol: str) -> dict[str, Any]:
    """离线校验：不发任何网络请求，只验证 convert_to_chat_completions 在一个具代表性的
    复杂多轮请求（system + 多轮 user/assistant + 工具调用 + 工具结果）下，
    是否会把某些内容悄悄丢掉。

    做法：构造一份源协议请求，每个位置都塞一个独一无二的标记字符串，转换完成后
    检查每个标记是否还能在输出里找到。这比只测"能不能转换成功"更能捕捉到
    "转换是成功了，但某个 content block 类型没处理、内容被吞掉了"这类问题。
    """
    if native_protocol == "chat_completions":
        return {"ok": True, "skipped": True, "reason": "原生协议已是 chat_completions，无需转换"}
    if native_protocol not in SOURCE_PROTOCOLS:
        return {"ok": False, "error": f"未知协议: {native_protocol}"}

    markers = {
        "system": "FIDELITY-MARK-SYSTEM-7f3a",
        "user1": "FIDELITY-MARK-USER1-9c2d",
        "assistant_text": "FIDELITY-MARK-ASSISTANT-1e8b",
        "tool_arg": "FIDELITY-MARK-ARG-4d6f",
        "tool_result": "FIDELITY-MARK-RESULT-2b9a",
        "user2": "FIDELITY-MARK-USER2-6a1c",
    }

    if native_protocol == "anthropic_messages":
        source: dict[str, Any] = {
            "model": "probe-model",
            ...
            "messages": [
                {"role": "user", "content": markers["user1"]},
                {
                    "role": "assistant",
                    "content": [
                        {"type": "text", "text": markers["assistant_text"]},
                        {
                            "type": "tool_use",
                            "id": "call_1",
                            "name": "lookup",
                            "input": {"query": markers["tool_arg"]},
                        },
                    ],
                },
                {
                    "role": "user",
                    "content": [
                        {"type": "tool_result", "tool_use_id": "call_1", "content": markers["tool_result"]},
                    ],
                },
                {"role": "user", "content": markers["user2"]},
            ],
            "tools": [
                {"name": "lookup", "description": "查", "input_schema": {"type": "object", "properties": {}}},
            ],
        }
    else:  # responses
        source = {
            ...
            "input": [
                {"type": "message", "role": "user", "content": [{"type": "input_text", "text": markers["user1"]}]},
                {
                    "type": "message",
                    "role": "assistant",
                    "content": [{"type": "output_text", "text": markers["assistant_text"]}],
                },
                {
                    "type": "function_call",
                    "call_id": "call_1",
                    "name": "lookup",
                    "arguments": json.dumps({"query": markers["tool_arg"]}),
                },
                {"type": "function_call_output", "call_id": "call_1", "output": markers["tool_result"]},
                {"type": "message", "role": "user", "content": [{"type": "input_text", "text": markers["user2"]}]},
            ],
            "tools": [
                {"type": "function", "name": "lookup", "description": "查", "parameters": {"type": "object", "properties": {}}},
            ],
        }

    try:
        converted = convert_to_chat_completions(native_protocol, source)
    except ProtocolAdaptError as exc:
        return {"ok": False, "error": f"转换失败: {exc}"}

    haystack = json.dumps(converted, ensure_ascii=False)
    missing = [label for label, marker in markers.items() if marker not in haystack]
    return {
        "ok": not missing,
        "native_protocol": native_protocol,
        "checked_markers": list(markers.keys()),
        "missing": missing,
        "message_count": len(converted.get("messages") or []),
        "has_tools": bool(converted.get("tools")),
    }
